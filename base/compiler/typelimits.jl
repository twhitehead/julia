...
function isnarrowenoughtype(@nospecialize t)
...
function tmerge(@nospecialize(typea), @nospecialize(typeb))
    suba = typea ⊑ typeb
    suba && isnarrowenoughtype(typeb) && return typeb
    subb = typeb ⊑ typea
    suba && subb && return typea
    subb && isnarrowenoughtype(typea) && return typea
...
function tuplemerge(a::DataType, b::DataType)
    ...
    for i = 1:lt
        ui = Union{ap[i], bp[i]}
        p[i] = isnarrowenoughtype(ui) ? ui : Any
    end
